...
- **请求参数**:
  ```json
  {
    ...
    "id_only": false,     // 是否只返回ID
    "start": 0,           // 分页起始位置
    "size": 75,           // 每页数量
    "date_key": "",       // 日期过滤: 2024(年) / 202401(月) / 1000001(活动ID+1000000)
    "path": "",           // 路径过滤
    "tag": "",            // 标签过滤
    "face_id": 0,         // 人脸ID过滤
...
- **说明**: 
  - 上传的文件会记录上传用户信息，从 HTTP Header 的 `Account` 字段获取
  - 上传后会在 `t_files` 表中记录文件信息，包括 `upload_user` 字段
- **响应**:
  ```json
  {
    "code": 0,
