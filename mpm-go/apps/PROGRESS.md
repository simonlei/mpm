...
5. ⬜ 创建WorkManager任务
   - 创建PeriodicWorkRequest定期同步任务
   - 根据用户配置的频率执行同步
   - 支持约束条件（仅WiFi、充电时、电量充足）
   - 支持重试策略（指数退避）
...
8. ⬜ 创建DirectoryPickerDialog
   - 使用DocumentsContract选择目录
   - 显示目录路径
   - 支持选择多个目录
   - 显示目录下的文件数量预览

9. ⬜ 创建SyncHistoryScreen
   - 显示同步历史记录列表
   - 显示文件名、大小、同步时间、状态
   - 支持按状态筛选（全部/成功/失败）
   - 支持重试失败的文件
...
10. ⬜ 实现文件变化监听
    - 使用ContentObserver监听MediaStore变化
    - 检测新增的照片和视频
    - 自动触发同步任务
    - 避免重复上传
...
### 🔥 立即开始 (任务14.5 - 高优先级)
**任务14.5：实现自动同步照片功能**
1. 扩展PreferencesManager，添加自动同步配置
2. 创建Room数据库实体（SyncFile、SyncDirectory）
3. 创建SyncRepository，实现文件扫描和同步逻辑
4. 创建PhotoSyncService后台服务
5. 创建WorkManager定期同步任务
6. 扩展SettingsScreen，添加自动同步配置界面
7. 创建DirectoryPickerDialog，支持选择同步目录
8. 创建SyncHistoryScreen，显示同步历史
9. 实现文件变化监听和自动同步
10. 实现同步通知和进度显示
