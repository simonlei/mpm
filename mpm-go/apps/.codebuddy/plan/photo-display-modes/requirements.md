...
9. **WHEN 用户在横屏模式下** THEN 系统 SHALL 采用左右分栏布局：左侧显示目录树（30%宽度），右侧显示照片网格（70%宽度）
10. **WHEN 用户在竖屏模式下** THEN 系统 SHALL 采用面包屑导航 + 底部抽屉布局：默认只显示面包屑和照片网格，点击面包屑的目录图标展开底部抽屉显示完整目录树
11. **WHEN 用户在竖屏模式下点击面包屑的目录图标** THEN 系统 SHALL 展开底部抽屉（占屏幕60-70%高度），显示完整的目录树
12. **WHEN 用户在底部抽屉中选择目录后** THEN 系统 SHALL 自动收起抽屉，更新面包屑导航和照片网格内容
...
#### 横屏模式（Landscape）：左右分栏布局
```
┌─────────────────────────────────────────────────┐
│  TopAppBar (面包屑导航 + 操作按钮)                │
├──────────────┬──────────────────────────────────┤
│              │                                  │
│  目录树区域   │      照片网格区域                 │
│  (30%宽度)   │      (70%宽度)                   │
...
**横屏模式特点**：
- ✅ 左侧目录树固定显示，支持滚动和展开/折叠
- ✅ 右侧照片网格占据主要空间
- ✅ 点击目录树中的文件夹，右侧照片网格实时更新
- ✅ 当前选中的目录高亮显示（使用Primary颜色背景）

#### 竖屏模式（Portrait）：面包屑 + 底部抽屉布局
```
// 默认状态：只显示面包屑和照片网格
┌─────────────────────────────────────┐
│  TopAppBar                          │
...
- ✅ 支持拖拽手势控制抽屉高度（60-70%）
- ✅ 点击目录后自动收起抽屉，体验流畅
- ✅ 符合Material Design的ModalBottomSheet规范

**实现技术要点**：
```kotlin
// 使用Configuration判断横竖屏
...
if (isLandscape) {
    // 横屏：左右分栏布局
    Row {
        FolderTreePanel(modifier = Modifier.weight(0.3f))
        Divider(modifier = Modifier.width(1.dp))
        PhotoGridPanel(modifier = Modifier.weight(0.7f))
    }
} else {
    // 竖屏：面包屑 + 底部抽屉
    Box {
        ...
        // 底部抽屉（使用ModalBottomSheet）
        if (showFolderDrawer) {
            ModalBottomSheet(
                onDismissRequest = { showFolderDrawer = false }
            ) {
                FolderTreePanel()
            }
        }
