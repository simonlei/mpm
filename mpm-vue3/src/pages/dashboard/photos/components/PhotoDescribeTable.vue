...
if (photo != null) {
  ...
  data.value.push({ k: '时间', v: photo.taken_date });
...
<template>
  <t-layout>
    <t-content>
      <t-base-table :key="photo?.id" :columns="columns" :data="data" :show-header=false bordered row-key="index"
        stripe></t-base-table>
    </t-content>
    <t-footer>
      <t-space align="center" direction="vertical">
        <t-space>
          ...
          <t-popup content="以当前照片创建活动" trigger="hover">
            <t-button :disabled="photo?.activity != null" shape="circle" variant="outline" @click="createActivity">
              <location-icon slot="icon" />
            </t-button>
          </t-popup>
          <t-popup v-if="filterStore.faceId != null" content="设置当前为默认人脸" trigger="hover">
            <t-button shape="circle" variant="outline" @click="setDefaultFace">
        ...
        <t-space>
          <t-popup v-if="detailViewStore.detailVisible" content="圈人" trigger="hover">
            <t-button shape="circle" variant="outline" @click="changeCircleFace">
              <face-retouching-icon slot="icon" />
            </t-button>
          </t-popup>
          <t-popup v-if="detailViewStore.detailVisible" content="重新识别人脸" trigger="hover">
            <t-button shape="circle" variant="outline" @click="rescanFaceInPhoto">
