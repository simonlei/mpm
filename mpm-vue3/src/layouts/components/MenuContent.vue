  ...
  <div>
    <template v-for="item in list" :key="item.path">
      <template v-if="!item.children || !item.children.length || item.meta?.single">
        <t-menu-item v-if="getHref(item)" :name="item.path" :value="getPath(item)"
      ...
      <t-submenu v-else :name="item.path" :title="item.title" :value="item.path">
        <template #icon>
          <t-icon v-if="beIcon(item)" :name="item.icon"/>
          <component :is="beRender(item).render" v-else-if="beRender(item).can" class="t-icon"/>
        </template>
...
const getPath = (item) => {
  if (active.value != null && active.value.startsWith(item.path)) {
    return active.value;
  }
